...
const doHUP = async (that, test, mode, hostapp, target) => {
	...
	switch (mode) {
		case 'local':
			...
			// delete tmp mount/umount steps from hostapp-update if they exist
			await that.worker.executeCommandInHostOS(
				`mount -o rw,remount / && sed '/LOADTMP/d' -i $(command -v hostapp-update) && mount -o ro,remount /`,
				target,
			);

			test.comment(`Running: hostapp-update -f ${hostappPath}`);
			hupLog = await that.worker.executeCommandInHostOS(`bash -x $(command -v hostapp-update) -f ${hostappPath}`, target);
